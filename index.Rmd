...
* un [*mappage* de la structure de données avec l'iso-standard CIDOC-CRM](#cidoc)  
...
htmltools::includeHTML(paste0(getwd(),"/img/site_GIIAB_IIIA1.html"))
```
...
htmltools::includeHTML(paste0(getwd(),"/img/site_GIIIA1_IIIA3.html"))
```
...
htmltools::includeHTML(paste0(getwd(),"/img/site_GIIIA3.html"))
```
...
Basé sur un wed de données décrites sémantiquement (web sémantique). Les objets numériques sont décrits généralement sous la forme de triplets RDF sous la forme: sujet-prédicat-objet (*subject-predicate-object*).
...
```{r UtoKulm, echo=FALSE, warning=FALSE, fig.cap="Uto-Kulm durant le (GIIAB-IIIA1)"}
chrono <- f.chrono(chm)
data <- f.data(chm)
tab <- cbind(data,chrono) # associe les tableaux
df.per <- f.a.period(tab,site.type,per) # subset # période 1
...
R offre différents package ([rdflib](https://cran.r-project.org/web/packages/rdflib/vignettes/rdf_intro.html), etc.) permettant de formater les données de l'étude: 
...
Le graph peut être sérialisé sous le format JSON-LD est actuellement le plus utilisé pour décrire des données web. 
...
Ce fichier peut ensuite être lu sur internet:

```{r json.read, echo=TRUE, fig.cap=" JSON"}

```

## Mappage CIDOC-CRM {#cidoc}

L'iso-standard [CIDOC-CRM](https://www.iso.org/standard/34424.html) (*Cultural Heritage Information-Conceptual Reference Model*, ISO 21127) est une ontologie pour la description et l’organisation de l’information liée au patrimoine archéologique et architectural.  
L'utilisation d'une ontologie permet l'interopérabilité des données et leur référencement sur le web sémantique, comme préconisé par l'*Open Science*.

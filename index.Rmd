...
Basé sur un wed de données décrites sémantiquement (web sémantique). Les objets numériques sont décrits généralement sous la forme de triples RDF sous la forme: sujet-prédicat-objet (*subject-predicate-object*).
...
Le triple peut être sérialisé sous le format JSON-LD est actuellement le plus utilisé pour décrire des données web. 
...
Les champs et les valeurs de la base de données seront alignées sur le [CIDOC-CRM](https://www.iso.org/standard/34424.html) (*Cultural Heritage Information-Conceptual Reference Model*, ISO 21127), un iso-standard pour la description et l’organisation de l’information liée au patrimoine archéologique et architectural. Son utilisation permet l'interopérabilité des données et leur référencement sur le web sémantique, comme préconisé par l'*Open Science*.

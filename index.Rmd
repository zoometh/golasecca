...
f.ggraph  <- function(df.per,obj.colors){
  ## nodes
  col.nds <- c("sites","contexte.site","contexte.forme.ply","contexte.couleur")
  # sites
  ...
  # g <- set.vertex.attribute(g, "contexte.forme.ply", value=df.per$contexte.forme.ply)
  ...
  #   if(!is.na(id.obj)){
  #     g <- set.vertex.attribute(g, "contexte.forme.ply", index = c(id.obj), "circle")
...
fig.count <- tab.count.melt %>%
  plot_ly(x = ~variable,
          ...
          name = ~objets,
          color = ~rgb) %>%
  add_annotations(x=period.objets.sums$period, y=period.objets.sums$x,
...
df.per <- merge(df.per,site.type,by="contexte.site",all.x=T) # merge
df.per <- df.per[, c("sites","objets","contexte.forme.ply","contexte.couleur","contexte.site" )]
fig <- f.ggraph(df.per,obj.colors)
fig
```
